...
define(['fs', 'path', 'typescript/lib/tsserverlibrary', 'vscode-languageserver', 'vscode-uri'], function (fs, path, tsserverlibrary, vscodeLanguageserver, vscodeUri) { 'use strict';
  ...
  `;
	...
	var cmdline_utils_1 = cmdline_utils.parseCommandLine;
	var cmdline_utils_2 = cmdline_utils.generateHelpMessage;
	...
	function tsCompletionEntryToLspCompletionItem(entry, position, scriptInfo) {
	    ...
	    item.textEdit = entry.replacementSpan ?
	        vscodeLanguageserver.TextEdit.replace(utils.tsTextSpanToLspRange(scriptInfo, entry.replacementSpan), insertText) :
	        vscodeLanguageserver.TextEdit.insert(position, insertText);
	    return item;
	}
	    ...
	    handleProjectServiceEvent(event) {
	        switch (event.eventName) {
	            ...
	            case tsserverlibrary.server.ProjectLoadingFinishEvent: {
	                ...
	                this.connection.sendNotification(protocol.projectLoadingNotification.finish, project.projectName);
	                break;
	            }
	    ...
	    checkIsAngularProject(project) {
	        ...
	        if (!project.languageServiceEnabled) {
	            const msg = `Language service is already disabled for ${projectName}. ` +
	                `This could be due to non-TS files that exceeded the size limit (${tsserverlibrary.server.maxProgramSizeForNonTsFiles} bytes).` +
	...
	var version_provider_1 = version_provider.resolveTsServer;
	var version_provider_2 = version_provider.resolveNgLangSvc;
	var version_provider_3 = version_provider.Version;
	...
	const ng = version_provider.resolveNgLangSvc(options.ngProbeLocations);
